...
def motors_velocity(il,dist):
	...
	if 3 < il <= midpos-d_negro:	# completamente sobre linea negra
		motor_v_L =  motor_base 
		motor_v_R = -motor_base

	if midpos-d_negro < il <= midpos:		# robot desvia un poco a izquierda
		print('black')
	...
	if midpos < il < midpos+d_blanco:	# robot desvia a derecha
		print('white')
		correccion = (midpos-il)/midpos
	...
	if il > midpos+d_blanco:	# completamente sobre linea negra
		motor_v_L= -motor_base 
		motor_v_R=  motor_base
